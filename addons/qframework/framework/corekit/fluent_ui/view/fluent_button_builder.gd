...
func width(_width:float) -> FluentButtonBuilder:
	var s = func(button:Button): button.size.x = _width
	property_setters.append(s)
	return self
...
func font_align(align:int)->FluentButtonBuilder:
	var s = func(button:Button):  \
		button.add_theme_constant_override("alignment",align) 
