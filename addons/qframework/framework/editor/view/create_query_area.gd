...
class_name CreateQueryArea

var query_name_input:TextEdit = null
...
func _on_create_button_pressed():
	GenerateQueryCode.generate(query_name_input.text,editor_plugin) 
	_clear_data()
	editor_plugin.show_main()
	editor_plugin.hide_bottom_panel()
	
func _clear_data():
	query_name_input.text = ""
	_update_preview()
	
func _update_preview():
	if query_name_input.text == "":  
		preview_label.text = ""  
		preview_path_label.text = GenerateQueryCode.DIR
		btn_create.hide()
	else: 
		preview_label.text = query_name_input.text + "Query" 
		preview_path_label.text = GenerateQueryCode.build_file_path(query_name_input.text)
		btn_create.show()

func build():
	
	root_view = (FluentUI.vbox() 
		.child(FluentUI.label().text("创建 Query").h_aligh_center().font_size(36))
		.child(FluentUI.hbox()
			.child(FluentUI.label().text("名字:"))
			.child(FluentUI.text_edit()
				.text_changed(func(): _update_preview()) 
				.min_size(300,20) 
				.on_build(func(t): query_name_input = t )
			)
