...
class_name CreateModelArea

var model_name_input:TextEdit = null
...
func _on_create_button_pressed():
	GenerateModelCode.generate(model_name_input.text,editor_plugin,indie_folder) 
	_clear_data()
	editor_plugin.show_main()
	editor_plugin.hide_bottom_panel()
	
func _clear_data():
	model_name_input.text = ""
	_update_preview()
	
func _update_preview():
	if model_name_input.text == "":  
		preview_label.text = ""  
		preview_path_label.text = GenerateModelCode.DIR
		btn_create.hide()
	else: 
		preview_label.text = model_name_input.text + "Model" 
		preview_path_label.text = GenerateModelCode.build_file_path(model_name_input.text,indie_folder)
		btn_create.show()

func build():
	root_view = (FluentUI.vbox() 
		.child(FluentUI.label().text("创建 Model").h_aligh_center().font_size(36))
		.child(FluentUI.hbox()
			.child(FluentUI.label().text("名字:")) 
			.child(FluentUI.text_edit() 
				.text_changed(func(): _update_preview()) 
				.min_size(300,20) 
				.on_build(func(t): model_name_input = t )
			) 
