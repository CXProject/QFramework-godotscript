...
class_name CreateCommandArea

var command_name_input:TextEdit = null
...
func _on_create_button_pressed():
	GenerateCommandCode.generate(command_name_input.text,editor_plugin) 
	_clear_data()
	editor_plugin.show_main()
	editor_plugin.hide_bottom_panel()
	
func _clear_data():
	command_name_input.text = ""
	_update_preview()
	
func _update_preview():
	if command_name_input.text == "":  
		preview_label.text = ""  
		preview_path_label.text = GenerateCommandCode.DIR
		btn_create.hide()
	else: 
		preview_label.text = command_name_input.text + "Command" 
		preview_path_label.text = GenerateCommandCode.build_file_path(command_name_input.text)
		btn_create.show()

func build():
	
	root_view = (FluentUI.vbox() 
		.child(FluentUI.label().text("创建 Command").h_aligh_center().font_size(36))
		.child(FluentUI.hbox()
			.child(FluentUI.label().text("名字:"))
			.child(FluentUI.text_edit()
				.text_changed(func(): _update_preview()) 
				.min_size(300,20) 
				.on_build(func(t): command_name_input = t )
			)
