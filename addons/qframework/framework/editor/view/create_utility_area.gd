...
class_name CreateUtilityArea

var utility_name_input:TextEdit = null
...
func _on_create_button_pressed():
	GenerateUtilityCode.generate(utility_name_input.text,editor_plugin,indie_folder) 
	_clear_data()
	editor_plugin.show_main()
	editor_plugin.hide_bottom_panel()
	
func _clear_data():
	utility_name_input.text = ""
	_update_preview()
	
func _update_preview():
	if utility_name_input.text == "":  
		preview_label.text = ""  
		preview_path_label.text = GenerateUtilityCode.DIR
		btn_create.hide()
	else: 
		preview_label.text = utility_name_input.text + "Utility" 
		preview_path_label.text = GenerateUtilityCode.build_file_path(utility_name_input.text,indie_folder)
		btn_create.show()

func build():
	root_view = (FluentUI.vbox() 
		.child(FluentUI.label().text("创建 Utility").h_aligh_center().font_size(36))
		.child(FluentUI.hbox()
			.child(FluentUI.label().text("名字:")) 
			.child(FluentUI.text_edit() 
				.text_changed(func(): _update_preview()) 
				.min_size(300,20) 
				.on_build(func(t): utility_name_input = t )
			) 
